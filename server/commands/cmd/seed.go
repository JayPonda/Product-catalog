...
func seedCmd(cfg AppConfig, logger *utils.StructuredLogger) *cobra.Command {
	cmd := &cobra.Command{
		...
		usersW.Write([]string{"id", "first_name", "last_name", "email"})
		...
		ordersW.Write([]string{"order_id", "user_id", "customer_id", "total_bill", "created_at", "updated_at"})
		...
		for _, row := range rows[1:] {
			...
			if _, seen := userUUIDs[code]; !seen {
				...
				usersW.Write([]string{userID.String(), code, code, code + "@seed.local"})
			}
