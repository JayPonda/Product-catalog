...
func seedCmd(cfg AppConfig, logger *utils.StructuredLogger) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			logger.Info(ctx, "seed.go", "RunE", "seed started", utils.LoggerMeta{"csv": seedCSV})

			// --- 1. read the source CSV ---
			f, err := os.Open(seedCSV)
			if err != nil {
				logger.Error(ctx, "seed.go", "RunE", "failed to open csv", utils.LoggerMeta{"path": seedCSV}, err.Error())
			...
			if err != nil {
				logger.Error(ctx, "seed.go", "RunE", "failed to parse csv", nil, err.Error())
				return fmt.Errorf("failed to parse csv: %w", err)
			}
			if len(rows) < 2 {
				logger.Warn(ctx, "seed.go", "RunE", "csv has no data rows", nil)
				return fmt.Errorf("csv has no data rows")
			}
			header := rows[0]
			if len(header) < 3 {
				logger.Warn(ctx, "seed.go", "RunE", "csv missing required columns", nil)
			...
			logger.Info(ctx, "seed.go", "RunE", "seed completed", utils.LoggerMeta{"users_created": usersCreated, "orders_created": ordersCreated})
			return nil
		},
	}
