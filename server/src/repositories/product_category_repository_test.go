...
func linkFixture(t *testing.T, pcRepo *repositories.ProductCategoryRepository, catRepo *repositories.CategoryRepository) (uuid.UUID, uuid.UUID) {
	t.Helper()
	cat, err := catRepo.CreateCategory(models.Category{Name: "fixture"})
	if err != nil {
		t.Fatal(err)
	}
	productID := uuid.New()
	if err := pcRepo.LinkCategory(productID, cat.ID); err != nil {
...
func TestProductCategoryRepo_Link_AndFetch_E2E(t *testing.T) {
	...
	links, err := pcRepo.GetCategoriesByProduct(productID)
	if err != nil {
		t.Fatal(err)
	}
	...
	byIds, err := pcRepo.GetCategoriesByProductIds([]uuid.UUID{productID, uuid.New()})
	if err != nil {
		t.Fatal(err)
	}
	if len(byIds) != 1 {
	...
	found, ok, err := pcRepo.GetProductCategory(productID, catID)
	if err != nil || !ok {
...
func TestProductCategoryRepo_Unlink_E2E(t *testing.T) {
	...
	if err := pcRepo.UnlinkCategory(productID, catID); err != nil {
		t.Fatal(err)
	}
	links, _ := pcRepo.GetCategoriesByProduct(productID)
	if len(links) != 0 {
		t.Errorf("unlinked category still visible: %+v", links)
	}

	// GetProductCategory sees the soft-deleted row (found=true, DeletedAt valid).
	row, ok, err := pcRepo.GetProductCategory(productID, catID)
	if err != nil || !ok {
...
func TestProductCategoryRepo_Relink_Reactivates_E2E(t *testing.T) {
	...
	if err := pcRepo.LinkCategory(productID, catID); err != nil {
		t.Fatal(err)
	}
...
func TestProductCategoryRepo_DeleteAllForProduct_E2E(t *testing.T) {
	...
	c1, err := catRepo.CreateCategory(models.Category{Name: "one"})
	if err != nil {
		t.Fatal(err)
	}
	c2, err := catRepo.CreateCategory(models.Category{Name: "two"})
	if err != nil {
		t.Fatal(err)
	}
	productID := uuid.New()
	...
	for _, cid := range []uuid.UUID{c1.ID, c2.ID} {
		if err := pcRepo.LinkCategory(productID, cid); err != nil {
			t.Fatal(err)
		}
		if err := pcRepo.LinkCategory(other, cid); err != nil {
			t.Fatal(err)
		}
	}

	if err := pcRepo.DeleteProductCategories(productID); err != nil {
		t.Fatal(err)
	}

	mine, _ := pcRepo.GetCategoriesByProduct(productID)
	if len(mine) != 0 {
		t.Errorf("expected all links for product removed, got %d", len(mine))
	}
	othersLinks, _ := pcRepo.GetCategoriesByProduct(other)
